 ...
 * vault dev server proxying to it. Both server binaries must be built up
 * front (`cargo build -p skb-server --bin skb-server --examples`) — cargo is
 * deliberately not invoked from inside Playwright.
 */
